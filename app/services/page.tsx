...
import { Plus, Pencil, Trash2, ClipboardList, Loader2, ChevronDown, CheckCircle2 } from 'lucide-react';
...
function offeringToService(o: { id: string; name: string; price: number; duration: number; description?: string; premium_hours?: { slots?: unknown[] }; promo?: { enabled?: boolean } }): BarberService {
  return {
    id: o.id,
    name: o.name,
    ...
    premiumHours: !!o.premium_hours && (Array.isArray((o.premium_hours as { slots?: unknown[] }).slots) ? (o.premium_hours as { slots: unknown[] }).slots.length >= 0 : true),
...
export default function ServicesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setListError('');
    try {
      const offerings = await listOfferings({ technician_id: technicianId, page_size: 100 });
      setServices(offerings.map(offeringToService));
    } catch (err) {
      setListError(err instanceof Error ? err.message : 'Failed to load services.');
      setServices([]);
    } finally {
      setListLoading(false);
    }
  ...
  const handleAddOrUpdate = async (e: React.FormEvent, onSuccess?: () => void) => {
    e.preventDefault();
    setFormError('');
    ...
    try {
      const promoPayload = offerPromotion
        ? { discount: 0, enabled: true, expiry: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10) }
        : undefined;
  ...
  const handleEdit = (s: BarberService) => {
    ...
    if (typeof window !== 'undefined' && window.innerWidth < 640) setMobileModalOpen(true);
  };
    ...
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-black pt-12 sm:pt-24 pb-8">
        <main className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 text-center">
            {isBarber ? (
              <>
                ...
                <button
                  ...
                  className="mt-4 px-4 py-2 text-sm font-medium text-zinc-900 dark:text-zinc-50 bg-zinc-200 dark:bg-zinc-700 rounded-lg hover:bg-zinc-300 dark:hover:bg-zinc-600 transition-colors"
                >
                  Try again
                </button>
              </>
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
              Add the services you offer and their prices. Clients will see this when you share your link.
            </p>
            {successMessage && (
              <p className="mt-3 flex items-center gap-2 text-sm text-green-700 dark:text-green-400" role="status">
          ...
          <div className="flex sm:hidden">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-4 py-3 rounded-xl text-sm font-medium bg-zinc-900 dark:bg-zinc-50 text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-colors"
            >
          ...
          >
            ...
            <form onSubmit={(e) => handleAddOrUpdate(e)} className="space-y-5">
              <div>
              ...
              <div>
                ...
                <p className="mt-1.5 text-xs text-zinc-500 dark:text-zinc-400">
                  Formatting: new lines, <strong>**bold**</strong>, and lists with <code className="px-1 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800">- item</code> or <code className="px-1 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800">* item</code>
                </p>
              </div>
              ...
              <div className="flex items-start justify-between gap-4 py-2">
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-zinc-900 dark:text-zinc-50">Premium Hours</p>
                  <p className="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">
                    Override the days and times you want this service to be bookable.
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors shrink-0 focus:outline-none ${
                    premiumHours ? 'bg-green-600 dark:bg-green-400' : 'bg-zinc-300 dark:bg-zinc-700'
                  }`}
                >
                  <span
              ...
              <div className="flex items-start justify-between gap-4 py-2">
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-zinc-900 dark:text-zinc-50">Offer Promotion</p>
                  <p className="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">
                    Offer this service at a discount for a certain period of time.
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors shrink-0 focus:outline-none ${
                    offerPromotion ? 'bg-green-600 dark:bg-green-400' : 'bg-zinc-300 dark:bg-zinc-700'
                  }`}
                >
                  <span
                    className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${
              ...
              <div className="flex flex-wrap items-center gap-2 pt-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium bg-zinc-900 dark:bg-zinc-50 text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-zinc-100 focus:ring-2 focus:ring-zinc-900 dark:focus:ring-zinc-50 focus:ring-offset-2 dark:focus:ring-offset-zinc-900 disabled:opacity-50 disabled:pointer-events-none transition-colors"
                >
          ...
          <section aria-labelledby="services-list-heading">
            ...
            {listError && (
              <div className="mb-3 flex flex-col sm:flex-row sm:items-center gap-2">
                <p className="text-sm text-red-600 dark:text-red-400 flex-1" role="alert">
                  {listError}
                </p>
                <button
                  type="button"
                  onClick={() => { setListError(''); load(); }}
            ...
            {listLoading ? (
              <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-8 flex items-center justify-center">
              ...
              <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-8 text-center">
                <ClipboardList className="w-12 h-12 mx-auto text-zinc-300 dark:text-zinc-600 mb-3" aria-hidden />
                <p className="text-sm text-zinc-600 dark:text-zinc-400">No services yet</p>
                <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-500">
                  <span className="hidden sm:inline">Use the form above to add your first service.</span>
                  <span className="sm:hidden">Tap Add service to add your first service.</span>
                </p>
                <button
                  type="button"
                  onClick={() => setMobileModalOpen(true)}
                  className="mt-4 sm:hidden inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium bg-zinc-900 dark:bg-zinc-50 text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-zinc-100"
                >
                  <Plus className="w-4 h-4" />
                  Add service
                </button>
              ...
              <ul className="space-y-2">
                {services.map((s) => (
                  <li
                    key={s.id}
                    className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 py-3 flex flex-row flex-wrap sm:flex-nowrap sm:items-center justify-between gap-3 shadow-sm"
                  >
                    {confirmRemoveId === s.id ? (
                      <>
                        <p className="text-sm text-zinc-700 dark:text-zinc-300 flex-1 min-w-0">
                          Remove <strong>{confirmRemoveName || s.name}</strong>?
                        </p>
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            type="button"
                            onClick={() => { setConfirmRemoveId(null); setConfirmRemoveName(''); }}
                        ...
                        <div className="min-w-0 flex-1 w-full sm:w-auto">
                          ...
                          <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-0.5 sm:mt-0">
                            <span>
                              ...
                              {s.duration && (
                                <span className="text-zinc-500 dark:text-zinc-500"> · {formatDuration(s.duration)}</span>
                              )}
                            </span>
                          </p>
                          {s.description && (
                            <div className="mt-1">
                              <RichText text={s.description} className="text-zinc-500 dark:text-zinc-500" />
                            </div>
                          )}
            ...
            {mobileModalOpen && (
              <div className="fixed inset-0 z-50 flex flex-col bg-white dark:bg-zinc-900">
                <div className="flex items-center justify-between shrink-0 px-4 py-3 border-b border-zinc-200 dark:border-zinc-800">
                  <button
                    type="button"
                    onClick={() => { setMobileModalOpen(false); clearForm(); }}
                ...
                <div className="flex-1 overflow-y-auto p-4">
                  ...
                  <form onSubmit={(e) => handleAddOrUpdate(e, () => setMobileModalOpen(false))} className="space-y-5">
                    <div>
                      <label htmlFor="modal-service-name" className={FORM_LABEL}>Name</label>
                      <input id="modal-service-name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Haircut, Beard trim" className={INPUT_BASE} autoComplete="off" />
                    </div>
                    <div>
                      <label htmlFor="modal-service-price" className={FORM_LABEL}>Price</label>
                      <div className="flex rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 overflow-hidden focus-within:ring-2 focus-within:ring-zinc-900 dark:focus-within:ring-zinc-50">
                        <span className="flex items-center px-3 text-zinc-500 dark:text-zinc-400 text-sm border-r border-zinc-300 dark:border-zinc-600 bg-zinc-50 dark:bg-zinc-800/50">$</span>
                        <input id="modal-service-price" type="text" inputMode="decimal" value={price} onChange={(e) => setPrice(toNumericPrice(e.target.value))} placeholder="0.00" className="flex-1 px-3 py-2 bg-transparent text-zinc-900 dark:text-zinc-50 text-sm min-w-0 focus:outline-none" autoComplete="off" />
                      </div>
                    </div>
                    <div>
                      <label htmlFor="modal-service-duration" className={FORM_LABEL}>Duration</label>
                      <div className="relative">
                        <select id="modal-service-duration" value={duration} onChange={(e) => setDuration(e.target.value)} className={`${INPUT_BASE} appearance-none pr-10`}>
                          {DURATION_OPTIONS.map((opt) => (
                    ...
                    <div>
                      <label htmlFor="modal-service-desc" className={FORM_LABEL}>Description</label>
                      <textarea id="modal-service-desc" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description. Use line breaks, **bold**, - or * for lists." rows={4} className={`${INPUT_BASE} resize-y min-h-[100px]`} />
                    </div>
                    <div className="flex items-start justify-between gap-4 py-2">
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-zinc-900 dark:text-zinc-50">Premium Hours</p>
                        <p className="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">Override the days and times you want this service to be bookable.</p>
                      </div>
                      <button type="button" role="switch" aria-checked={premiumHours} onClick={() => setPremiumHours((v) => !v)} className={`relative w-12 h-6 rounded-full transition-colors shrink-0 focus:outline-none ${premiumHours ? 'bg-green-600 dark:bg-green-400' : 'bg-zinc-300 dark:bg-zinc-700'}`}>
                        <span className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${premiumHours ? 'left-7' : 'left-1'}`} />
                      </button>
                    </div>
                    <div className="flex items-start justify-between gap-4 py-2">
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-zinc-900 dark:text-zinc-50">Offer Promotion</p>
                        <p className="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">Offer this service at a discount for a certain period of time.</p>
                      </div>
                      <button type="button" role="switch" aria-checked={offerPromotion} onClick={() => setOfferPromotion((v) => !v)} className={`relative w-12 h-6 rounded-full transition-colors shrink-0 focus:outline-none ${offerPromotion ? 'bg-green-600 dark:bg-green-400' : 'bg-zinc-300 dark:bg-zinc-700'}`}>
                        <span className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${offerPromotion ? 'left-7' : 'left-1'}`} />
                      </button>
                    </div>
                    <div className="flex gap-2 pt-2">
                      {editingId && (
                        <button type="button" onClick={() => { clearForm(); setMobileModalOpen(false); }} className="flex-1 px-4 py-2.5 rounded-lg text-sm font-medium text-zinc-600 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800">
                          Cancel
                        </button>
                      )}
                      <button type="submit" disabled={saving || !name.trim() || !toNumericPrice(price)} className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium bg-zinc-900 dark:bg-zinc-50 text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-zinc-100 disabled:opacity-50">
                        {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                        {saving ? 'Saving...' : editingId ? 'Update service' : 'Add service'}
                      </button>
                    </div>
