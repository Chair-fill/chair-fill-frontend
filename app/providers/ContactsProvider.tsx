...
export function ContactsProvider({ children }: { children: ReactNode }) {
  ...
  const addContacts = useCallback(
    async (newContacts: Omit<Contact, "id">[]) => {
      if (isDemoMode()) {
        const contactsWithIds: Contact[] = newContacts.map(
          (contact): Contact => {
  ...
  const uploadBulkFile = useCallback(
    async (file: File) => {
      if (isDemoMode()) {
        ...
        const { parseCSV, parseVCF } =
          await import("@/lib/utils/contact-parser");
      ...
      if (technicianId == null || technicianId === "") {
        throw new Error(
          "Barber profile is required to upload contacts. Complete Barber info in Account Settings.",
        );
      }
      await uploadContactsBulkFile(file, technicianId);
      // Let the backend process the file asynchronously before we fetch
