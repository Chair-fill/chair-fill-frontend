...
async function getRedirectAfterLogin(fallback: string): Promise<string> {
  try {
    ...
    const prog: ProgressPayload | undefined =
      raw && "data" in raw && raw.data != null
        ? raw.data
        : (raw as ProgressPayload | undefined);
    const isTechnician =
      prog && typeof prog === "object" && prog.is_technician === true;
    const hasSubscribed =
      prog && typeof prog === "object" && prog.has_subscribed === true;

    if (hasSubscribed) return fallback;
...
export default function LoginPage() {
  ...
  const { signinVerify, signinWithOtp, loginWithDemo, logout, isLoading } =
    useUser();
  ...
  const handleCredentialsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.needsOtp && result.verifyToken) {
        ...
        try {
          const fallback = redirectTo.startsWith("/")
            ? redirectTo
            : DEFAULT_REDIRECT;
  ...
  if (isRedirecting) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-sm text-foreground/70">Signing you in...</p>
      </div>
    );
  }
            ...
            {isDemoAvailable && (
              <>
                ...
                <button
                  ...
                  className="w-full py-3 px-6 border-2 border-border text-foreground rounded-full hover:bg-foreground/5 font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all"
                >
                  {demoLoading ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" />
